...
class Main {
  public static void main(String[] args) {
    ...
    Map<String, Integer> dominated = new HashMap<String, Integer>();
    dominated.put("A", 11);
    dominated.put("K", 4);
    dominated.put("Q", 3);
    dominated.put("J", 2);
    ...
    for (int index = 0; index < 4*hands; index++) {
      ...
      if (cardSuit.equals(suit)) {
        if (dominant.containsKey(cardValue)) total += dominant.get(cardValue);
      }
      else {
        if (dominated.containsKey(cardValue)) total += dominated.get(cardValue);
      }
    }
