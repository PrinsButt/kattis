...
class Main {
  ...
  public static void main(String[] args) {
    ...
    if (power >= Victory.PROVINCE.cost()) bestVictory = Victory.PROVINCE;
    else if (power >= Victory.DUCHY.cost()) bestVictory = Victory.DUCHY;
    else if (power >= Victory.ESTATE.cost()) bestVictory = Victory.ESTATE;

    if (power >= Treasure.GOLD.cost()) bestTreasure = Treasure.GOLD;
    else if (power >= Treasure.SILVER.cost()) bestTreasure = Treasure.SILVER;
    else if (power >= Treasure.COPPER.cost()) bestTreasure = Treasure.COPPER;

    if (bestVictory != null) System.out.println(bestVictory.display() + " or " + bestTreasure.display());
    else System.out.println(bestTreasure.display());
  }
}